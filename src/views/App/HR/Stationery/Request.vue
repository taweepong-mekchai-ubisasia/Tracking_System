                ...
                <table class="table table-sm" cellpadding="0" cellspacing="0" width="100%">
                  <thead>
                    <tr>
                      ...
                      <td>หมายเหตุ</td>
                      <td v-if="!canedit">บันทึกแก้ไข/ลบ</td>
                      <td v-else>บันทึกแก้ไข</td>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(row, index) in detail.rows" :key="index" v-if="base.controll == 'edit'">
                      ...
                      <td width="15%"><input v-model="row.item_name" type="text" placeholder="Typing to find item ..." class="input input-sm input-bordered w-full" :class="row.status ? 'bg-gray-200' : ''" :readonly="row.status" /></td>
                      <td width="10%"><input v-model="row.form_item_amount" type="number" class="input input-sm input-bordered w-full" :class="row.status ? 'bg-gray-200' : ''" :readonly="row.status" /></td>
                      ...
                      <td width="15%">
                        <select class="select select-bordered select-sm w-full border-gray-300" v-model="row.rack_id" :disabled="base.form.status != 'inprocess' || row.status">
                      ...
                      <td style="display: flex; flex-direction: column;">
                        <button class="btn btn-xs btn-outline btn-success w-full" :class="row.status == 'ready' ? 'btn-active' : ''" @click="detect_rack_field('ready', index)" :disabled="base.form.status != 'inprocess' || row.status">Ready</button>
                        <button class="btn btn-xs btn-outline btn-warning w-full mt-1" :class="row.status == 'reject' ? 'btn-active' : ''" @click="detect_rack_field('reject', index)" :disabled="base.form.status != 'inprocess'  || row.status">Reject</button>
                      </td>
                      ...
                      <td width="20%"><input v-model="row.item_detail" type="text" class="input input-sm input-bordered w-full" :class="row.status ? 'bg-gray-200' : ''" :readonly="row.status" /></td>
                      <td width="5%" class="text-center">
                        <button class="btn btn-xs btn-warning text-white" :disabled="row.status"><Icon icon="bi:pencil-square" width="14" height="14" /></button>
                        <button class="btn btn-xs btn-error text-white ml-1" :class="canedit ? 'hidden' : ''"><Icon icon="bi:trash3" width="14" height="14" /></button>
                      </td>
                    </tr>
            ...
            <div class="grid grid-cols-2">
              <div class="grid grid-rows-2 text-left">
                <div class="flex items-center text-primary gap-1">
                  <Icon icon="bi:clipboard-check" width="20" height="20" />
                  <h3 class="text-xl font-bold">REQUEST LISTS</h3>
                </div>
                <h3 class="font-bold">รายการข้อมูลการร้องขอ / เบิก / จ่าย อุปกรณ์เครื่องใช้สำนักงาน</h3>
              </div>
              <div class="text-right">
                <label
                  ...
                  @click="base_create()"
                >
                  <Icon icon="bi:file-earmark-plus" width="16" height="16" />
                  ขอเบิกอุปกรณ์
                </label>
              </div>
            </div>
            <div class="mt-3 border-2 border-dashed rounded-xl p-2">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="flex justify-start join">
                  ...
                  >
                  <Icon icon="bi:list-task" width="16" height="16" />
                    Item Lists
                  </button>
                </div>
              ...
              <div class="overflow-x-auto w-full max-h-[48vh] my-2">
                <table class="table table-pin-rows table-pin-cols">
                  ...
                  <tbody class="text-xs">
                    <tr v-if="base.total" v-for="(row, i) in base.rows" style="cursor: pointer;" class="hover" :class="row.status == 'done' ? 'bg-gray-300' : row.status == 'cancelled' ? 'bg-red-200' : ''" @dblclick="base_edit(`${row.form_id}`, `${i}`)">
                      <td>{{ row.form_id }}</td>
                      <td>{{ row.form_type }}</td>
                      <td><span class="badge font-semibold text-xs" :class="row.status == 'cancelled' ? 'badge-error text-white' : ''">{{ row.status }}</span></td>
                      <td>{{ $moment(row.request_date).format("DD-MM-YYYY HH:MM:SS") }}</td>
                      <td>{{ $moment(row.request_due_date).format("DD-MM-YYYY HH:MM:SS") }}</td>
                      <td>{{ row.requester_name }}</td>
                      <td>{{ row.title }}</td>
                      <td>{{ row.approver_name  || '-' }}</td>
                      <td>{{ row.approve_date ? $moment(row.approve_date).format("DD-MM-YYYY HH:MM:SS") : '-' }}</td>
                      <td>{{ row.receiver_name || '-' }}</td>
                      <td>{{ row.receive_date ? $moment(row.receive_date).format("DD-MM-YYYY HH:MM:SS") : '-' }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
  ...
  methods: {
    ...
    detect_rack_field(status, i) {
      if (this.detail.rows[i].rack_id) {
        ...
        this.detail.form = { ...this.detail.rows[i] };
        this.detail_save('static')
      } else {
    ...
    rack_search(id) {
      new Query(null,'get').get(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/HR/sta/rack_items?item_id=${id}&total=1`, (res) => {
        if (res.success) {
    ...
    base_get(callback) {
      new Query('base','get').get(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/HR/sta/forms?total=1&page=${this.base.page}${this.base.row ? `&rows=${this.base.row}` : ""}${this.base.q ? `&q=${this.base.q}` : ""}`, (res) => {
        if (res.success) {
    ...
    base_edit(code, index) {
      ...
      if (this.base.form.status == 'waiting' && this.base.form.status == 'approve') {
        this.canedit = 0
      } else {
        this.canedit = 1
      }
    ...
    detail_save(type) {
        ...
        new Query(null, "PUT").set(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/HR/sta/forms_items`, obj, (res) => {
          if (!res.success) {
          } else {  
            new Query(null, "POST").set(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/HR/sta/forms_items`, obj, (res) => {
              if (!res.success) {
              } else {  
                if (type == "static") {
                  this.detail_search();
