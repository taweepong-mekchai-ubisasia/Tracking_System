<template>
  <AppLayout ref="AppLayoutComponent">
    <template #modal>
      ...
      />
      <AppModuleGlobalStatusPending
        v-model="pending"
        @update:success="fnPending.success"
        ref="statusPendingModal"
      />
    </template>
    <template #view>
            ...
            <div>
              ...
              <div :class="`${base.pagination.loading ? 'blur-sm' : ''}`">
                <div ref="titleZone">
                  ...
                  >
                    <h3 class="text-lg font-bold text-primary px-2 sm:px-4">REQUEST&nbsp;LISTS</h3>
                    <div
                ...
                >
                  ...
                  >
                    <thead>
                      <tr>
                        <td colspan="5" class="text-center bg-rose-400 text-sm">รายการ Booking</td>
                        <td colspan="5" class="text-center bg-blue-400 text-sm">รายการ Request</td>
                      </tr>
                      <tr>
                        <td width="10%" class="bg-rose-200" style="height: 26px">ผู้เยี่ยมชม</td>
                        <td width="10%" class="bg-rose-200" style="height: 26px">หัวข้อ</td>
                        <td width="10%" class="bg-rose-200" style="height: 26px">รายละเอียด</td>
                        <td width="10%" class="bg-rose-200" style="height: 26px">ผู้ทำรายการ</td>
                        <td width="10%" class="bg-rose-200" style="height: 26px">วันที่ต้องการ</td>
                        <td width="10%" class="bg-blue-200" style="height: 26px">สิ่งที่ต้องการ</td>
                        <td width="10%" class="bg-blue-200" style="height: 26px">จำนวน</td>
                        <td width="10%" class="bg-blue-200" style="height: 26px">หน่วย</td>
                        <td width="10%" class="bg-blue-200" style="height: 26px">หมายเหตุ</td>
                        <th width="10%" class="bg-blue-200 text-right" style="height: 26px">สถานะ</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr 
                        ...
                        :key="v.code" 
                        :class="v.process == 'cancel'  ? 'opacity-50 line-through decoration-rose-500' 
                          : v.process == 'acknowledge' ? 'text-green-700' 
                          : v.process == 'reject' ? 'text-orange-500' : ''"
                      >
                        <td class="font-bold">
                          {{ v.visitorTitle }}
                        </td>
                        <td>{{ v.titleName }}</td>
                        <td>
                          <div class="tooltip tooltip-bottom" :data-tip="v.description">
                            <div class="w-full">
                              {{ (v.description).match(/^.{0,50}/)[0] }}...
                            </div>
                          </div>
                        </td>
                        ...
                        <th class="text-right">
                          <span
                            v-if="
                              v.process == 'pending' && v.status != 'Completed'
                            "
                          >
                            <label
                            ...
                            >
                              <Icon
                                ...
                                class="w-3 h-4 block lg:hidden"
                              />
                              <span class="hidden lg:block">Acknowledge</span>
                            </label>
                            <label
                          ...
                          >
                            <button
                              class="join-item btn modal-button btn-xs disabled:text-success italic"
                              :class="i % 2 == 0 ? 'disabled:bg-base-100' : 'disabled:bg-base-200'"
                              disabled
                            >
                              <span>Completed</span>
                            </button>
                          ...
                          >
                            <label
                              ...
                              class="join-item btn btn-link text-gray-500 modal-button btn-xs"
                              @click="
                                fnCancel.item(
                                  v,
                                  'api/controllers/MYSQL/INTERNAL/HR/visitor/visitor'
                          ...
                          >
                            <button
                              class="join-item btn btn-link text-orange-500 modal-button btn-xs"
                              @click="
                                fnPending.item(
                                  v,
                                  'api/controllers/MYSQL/INTERNAL/HR/visitor/visitor'
                            ...
                            >
                              <Icon
                                icon="mage:inbox-cross"
                                class="w-4 h-4 block lg:hidden"
                              />
                              <span class="hidden lg:block">Cancel Rejection</span>
                            </button>
                          </span>
                          <span
                            v-else-if="v.process == 'cancel'"
                          >
                            <button
                              class="join-item btn modal-button btn-xs disabled:text-gray-500 italic"
                              :class="i % 2 == 0 ? 'disabled:bg-base-100' : 'disabled:bg-base-200'"
                              disabled
                            >
                              <span>Cancelled</span>
...
import AppModuleGlobalStatusPending from "@/components/App/Module/Global/StatusPending.vue";
  ...
  setup() {
    ...
    const fnPending = {
      item: (v, path) => {
        ...
        pending.value.path = path;
      },
      success: (res) => {
        pending.value.modal = false;
        fnBase.search();
      },
    };
