    ...
    <template #modal>
      ...
      <div class="modal">
        ...
        >
          ...
          <h3 class="text-lg font-bold text-primary">SHORT URL</h3>
                  ...
                  >
                    ...
                    <tbody>
                      ...
                      >
                        ...
                        <th class="text-right">
                          <a
                            :href="`${this.serviceUrl}short/?code=${v.code}`"
                            target="_blank"
                            class="btn btn-link text-info no-underline modal-button btn-xs"
                            >Link
                          </a>
                          <label
                          
                            class="btn btn-link text-secondary no-underline modal-button btn-xs"
                            @click="copy(`${this.serviceUrl}short/?code=${v.code}`)"
                            >Copy
                          </label>
                          <label
                            for="modal-base"
  ...
  methods: {
    ...
    base_get(callback) {
      new Query('base','get').get(this, `${
          this.serviceUrl
        }api/controllers/MYSQL/INTERNAL/System/short?total=1&page=${
          this.base.page
    ...
    base_save() {
      ...
      new Query('base', this.base.controll == "create" ? "POST" : "PUT").set(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/System/short`, obj, (res) => {
        if (!res.success) {
        // localStorage.removeItem("user_token");
        // this.$router.push({ name: `Login` });
        } else {
