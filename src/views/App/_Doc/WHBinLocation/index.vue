              ...
              <div :class="`${base.pagination.loading ? 'blur-sm' : ''}`">
                <div ref="titleZone">
                  ...
                  >
                    <button
                      ...
                      @click="file = 'คู่มือใช้งาน_Warehouse_Bin_Location'"
                      :disabled="file == 'คู่มือใช้งาน_Warehouse_Bin_Location'"
                    >
											คู่มือใช้งาน Warehouse Bin Location
                    </button>
                    |
                    <button
                      class="btn btn-xs btn-neutral"
                      @click="exportExcel('ITEM_MASTER_UBP_Rev02')"
                    >
                      <Icon icon="file-icons:microsoft-excel" />
                      Short Code Master UBP
                    </button>
                    <button
                      class="btn btn-xs btn-neutral"
                      @click="exportExcel('ITEM_MASTER_UBA_Rev02')"
                    >
                      <Icon icon="file-icons:microsoft-excel" />
                      Short Code Master UBA
                    </button>
                    <button
                      class="btn btn-xs btn-neutral"
                      @click="exportExcel('Rev_Data_IM_Binlocation_29-03-2024_(Update)')"
                    >
                      <Icon icon="file-icons:microsoft-excel" />
                ...
                >
                  <div style="height: 74vh" v-if="file">
                    ...
                    <PDFViewer
                      v-else
                      :source="`${serviceUrl}/api/controllers/MYSQL/INTERNAL/Global/doc?file=${file}.pdf&path=WHBinLocation`"
...
export default {
  ...
  setup() {
    ...
    const download = (source, filename) => {
      ...
      link.href = `${serviceUrl.value}/upload/_Doc/WHBinLocation/${file.value}.pdf`;
      link.target = "_blank";
    ...
    const exportExcel = (file) => {
      return window.open(
        `${serviceUrl.value}upload/_Doc/WHBinLocation/${file}.xlsx`
      );
    };
