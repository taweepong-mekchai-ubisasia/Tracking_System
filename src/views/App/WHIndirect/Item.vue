        ...
        <div class="card shadow-lg bg-base-100">
          <div class="card-body overflow-auto">
            ...
            <!-- <div :class="`${base.loading ? 'blur-sm' : ''}`"> -->
              <div class="flex justify-end" v-if="user.access.WHIndirect.WHIndirectItem != 'user'">
                <label
                  for="modal-base"
                  ...
                  <table class="table table-xs table-pin-rows table-pin-cols table-zebra" v-else>
                    <thead>
                      <tr class="italic">
                        ...
                        <th v-if="user.access.WHIndirect.WHIndirectItem != 'user'"></th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="(v, i) in base.rows" :key="v.code" class="hover">
                        ...
                        <th class="text-right" v-if="user.access.WHIndirect.WHIndirectItem != 'user'">
                          <label
                            for="modal-base"
                          ...
                          <label
                            ...
                            "
                            v-if="user.access.WHIndirect.WHIndirectItem == 'superadmin'"
                          >
                            <!-- {{ user.access.WHIndirect.WHIndirectItem }} -->
                            ลบ
                          </label>
                        </th>
                      </tr>
...
export default {
  ...
  methods: {
    ...
    base_get(callback) {
      let dept = ''
      if (this.user.depRef > 329 && this.user.depRef < 333) dept = 'rd'
      ...
      new Query('base','get').get(this, `${
          this.serviceUrl
        }api/controllers/MYSQL/INTERNAL/WHI/item?total=1${this.user.access.WHIndirect.WHIndirectItem != 'superadmin' ? `&owner=${dept}` : ''}&page=${
          this.base.page
    ...
    base_save() {
      if(!this.base.form.title || !this.base.form.min || !this.base.form.unit) {
      ...
      if (this.base.controll == 'create') {
        if (this.user.depRef > 329 && this.user.depRef < 333) obj['rows'][0]['owner'] = 'rd'
        else if (this.user.depRef == 221 || this.user.depRef == 228) obj['rows'][0]['owner'] = 'pd'
        else if (this.user.depRef == 222 || this.user.depRef == 229) obj['rows'][0]['owner'] = 'qc'
        else if (this.user.depRef == 779) obj['rows'][0]['owner'] = 'qa'
        else obj['rows'][0]['owner'] = 'im'
      }
