...
export default {
  ...
  methods: {
    exportExcel(controll) {
      if (controll == 'base') {
        return window.open(`${this.serviceUrl}api/controllers/SAP/Excel/${this.wh}/qc?for=fgreport&total=1
    ...
    base_get(callback) {
      new Query('base','get').get(this, `${this.serviceUrl}api/controllers/SAP/${this.search.wh}/QC/fg_inspec?total=1&page=${this.base.page}${this.search.customer ? `&customer=${this.search.customer}` : ""}
        ${this.search.pdNo.from ? `&pdNo_from=${this.search.pdNo.from}` : ''}${this.search.pdNo.to ? `&pdNo_to=${this.search.pdNo.to}` : ''}
        ${this.search.itemCode.from ? `&code_from=${this.search.itemCode.from}` : ''}${this.search.itemCode.to ? `&code_to=${this.search.itemCode.to}` : ''}
        ${this.search.lot.from ? `&lot_from=${this.search.lot.from}` : ""}${this.search.lot.to ? `&lot_to=${this.search.lot.to}` : ""}
        ${this.search.mfg.from ? `&start_date=${this.search.mfg.from}` : ''}${this.search.mfg.to ? `&end_date=${this.search.mfg.to}` : ''}
    ...
    detail_get(callback) {
      new Query('detail','get').get(this, `${this.serviceUrl}api/controllers/SAP/${this.wh}/QC/fg_inspec_result?total=1&doc=${this.base.form.DocEntry}`, (res) => {
        if (res.success) {
          res.rows.forEach((v, i) => {
            res.rows[i].image = v.image ? JSON.parse(v.image) : [];
            res.rows[i].master = 0;
