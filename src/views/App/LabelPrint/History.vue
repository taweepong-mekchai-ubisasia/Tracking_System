<template>
  <AppLayout>
    <template #modal>
      ...
      <div class="modal">
        <div
          class="modal-box relative w-11/12 lg:w-8/12 xl:w-8/12 max-w-5xl p-2 lg:p-4 max-h-screen"
        >
          <label
            for="modal-base"
          ...
          <h3 class="text-lg font-bold text-primary">LOGS</h3>
          <div class="divider my-1"></div>
          <div class="card-body overflow-auto max-h-[80vh] p-2 lg:p-4">
            <div class="form-control">
              <div class="grid place-items-center">
                <div v-for="(v, i) in base.form.Logs">
                  <span v-if="i == 'items'">
                    <pre>
                              {{ v }}
                    </pre></span
  ...
  methods: {
    ...
    base_get(callback) {
      new Query("base", "get").get(
        ...
        `${
          this.serviceUrl
        }api/controllers/MYSQL/INTERNAL/LabelPrint/history?total=1&page=${
          this.base.page
        ...
        (res) => {
          if (!res.success) {
            ...
            res.rows.forEach((v, i) => {
              res.rows[i].Logs = v.Logs ? JSON.parse(v.Logs) : [];
              // res.rows[i].master = 0;
              // JSON.parse(data[i].Logs)
            });
          }
          callback({ ...res });
        }
