        ...
        <div class="modal-box relative w-11/12 max-w-2xl">
          ...
          <div class="overflow-x-auto max-h-[60vh] my-5">
            <table class="table table-xs table-pin-rows table-pin-cols">
              ...
              <tbody>
                <tr v-for="(v, i) in customer.rows" :class="v.code == base.form.customer ? 'bg-primary text-white' : ''">
                  <th class="text-center"><input type="radio" name="radio-1" class="radio radio-xs" :value="v.code" v-model="base.form.customer" /></th>
                  <td>{{ v.eng_name }}</td>
          ...
          <div class="backdrop-blur sticky top-0 items-center gap-3 flex"> 
            <div class="flex-1 form-control">
              <label 
                ...
                :class="!base.form.customer ? 'btn-disabled' : ''"
                @click="customer_save(customer.rows.find(x => x.code == base.form.customer))"
              >
                Confirm
              </label>
            </div>
              ...
              <div class="overflow-x-auto w-full max-h-[60vh] my-3">
                ...
                <table class="table table-xs table-pin-rows table-pin-cols table-zebra" v-else>
                  ...
                  <tbody>
                    ...
                    >
                    <!-- :class="row.status == 'picking' ? 'text-rose-700' : ''" -->
                      <th>
                      ...
                      <td>
                        <span 
                          ...
                          :class="row.status == 'packing' ? 'badge-info' : 'badge-error'">
                            {{ row.status }}
                        </span>
                      </td>
                      <td>{{ row.customer }}</td>
                      <td class="italic">{{ $moment(row.sending_date).format("DD-MM-YYYY") }}</td>
                      <!-- <td><span class="badge badge-sm font-semibold italic" :class="row.status == 'picking' ? 'badge-error text-error-content' : i & 1 ? '' : 'badge-ghost'">{{ row.status }}</span></td> -->
                      <td>
                        <div class="flex items-center space-x-3">
                          <div>
                            <div>
                      ...
                      <th class="text-right">
                        <label
                          for="modal-base"
                          class="join-item btn btn-warning hover:text-white modal-button btn-xs w-20"
