...
export default {
  ...
  methods: {
    ...
    base_get(callback) {
      new Query('base','get').get(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WMS/vita?status=packing&total=1&page=${this.base.page}${this.base.row ? `&rows=${this.base.row}` : ""}${this.base.q ? `&q=${this.base.q}` : ""}`, (res) => {
        if (res.success) {
    ...
    base_save(type) {
      ...
      new Query(null, 'put').set(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WMS/vita`, obj, (res) => {
        if (!res.success) {
          ...
          if (this.base.form.status == 'deliver') {
            let obj = { rows: [] }
