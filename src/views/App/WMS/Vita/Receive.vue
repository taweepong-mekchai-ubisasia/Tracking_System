              ...
              <div class="overflow-x-auto min-h-40 max-h-40 border">
                <table class="table table-xs table-pin-rows table-pin-cols table-zebra table-auto">
                  ...
                  <tbody>
                    ...
                    >
                      <td class="font-bold">
                        {{ index + 1 }} <span class="text-slate-500">({{ row.code }})</span>
                      </td>
                      <td>{{ row.product }}</td>
                      <td>{{ row.descrip }}</td>
                      ...
                      <td v-if="base.form.status == 'receiving' || base.form.status == 'received'">
                        <div class="flex items-center space-x-3">
                          <div class="italic">
                            <div>
                              date : {{ row.updated_at ? $moment(row.updated_at.split(' ')[0]).format("DD-MM-YYYY") : '-' }}
                            </div>
                            <div>
                      ...
                      <th class="text-right" v-else>
                        <label
                          :class="row.received ? 'btn btn-ghost modal-button btn-xs' : 'btn btn-disabled modal-button btn-xs'"
              ...
              <div style="display: flex; justify-content: center;" v-for="row in datalist" v-if="build">
                <div style="white-space: nowrap;" align="center">
                  ...
                  <table style="width: 100%">
                    <tr>
                      <td>
                        <VueQRCodeComponent :text="'http://192.168.11.1/WMS/Vita/ScanToReceive?'+detailcode+'&'+row.pallet+'&'+row.total+'&ubis'" :size="114" />
                      </td>
                      <td>
                ...
                <div class="overflow-x-auto w-full max-h-[55vh] my-2">
                  ...
                  <table class="table table-xs table-pin-rows table-pin-cols table-zebra" v-else>
                    ...
                    <tbody>
                      ...
                      >
                      <!-- :class="row.status == 'received' ? 'text-emerald-700' : row.status == 'receiving' ? 'text-amber-700' : ''" -->
                        <th>
                          <div class="flex items-center space-x-3">
                            <div>
                              <div>
  ...
  methods: {
      ...
      new Query('detail','get').get(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WMS/prod?trans_code=${this.base.form.code}&total=1`, (res) => {
        if (res.success) {
          ...
          res.rows.filter(x => x.code).forEach((x, i) => {
            ...
            if (Array.isArray(JSON.parse(x.format))) {
              ...
              checklist[i] = JSON.parse(x.format)
              x['already_have'] = checklist[i].total
            }
          }) 
        }
    ...
    item_get(callback) {
      new Query('item','get').get(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WMS/item?prod_code=${this.detail.form.code}&total=1`, (res) => {
        if (res.success) {
          if(this.detail.form.format) {
            let receive = JSON.parse(this.detail.form.format)
            let checklist = []
