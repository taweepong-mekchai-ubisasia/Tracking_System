        ...
        <template #modal>
            ...
            <div class="modal" v-if="base.modal">
                <div class="modal-box w-11/12 max-w-7xl">
                    ...
                    <h3 class="text-lg font-bold text-primary">Logs of Product : {{ base.form.product }}</h3>
                    <hr class="mt-5" />
                    ...
                    <div class="card-body overflow-auto">
                        ...
                        <div class="overflow-x-auto w-full max-h-[60vh] mt-1 border">
                            <table class="table table-xs table-pin-rows table-pin-cols table-zebra">
                                ...
                                <tbody>
                                    <tr v-if="base.total" v-for="(row, index) in base.rows" :key="row.code" :class="row.total_qty > '0' ? '' : 'text-slate-500'">
                                        <!-- <th>{{ index + 1 }}</th> -->
...
export default {
    ...
    methods: {
        ...
        base_get(callback) {
            new Query('base','get').get(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WMS/prod?vita=1&received=1&groupby=1&total=1&page=${this.base.page}${this.base.row ? `&rows=${this.base.row}` : ""}${this.base.q ? `&q=${this.base.q}` : ""}`, (res) => {
                if (res.success) {
                    // console.log(res)
        ...
        detail_get(callback) {
            new Query('detail','get').get(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WMS/logs?parent=${this.base.form.product}&descrip=${this.base.form.descrip}&total=1&page=${this.detail.page}${this.detail.row ? `&rows=${this.detail.row}` : ""}${this.detail.q ? `&q=${this.detail.q}` : ""}`, (res) => {
                if (res.success) {
                res.rows.forEach((v, i) => {
                    res.rows[i].image = v.image ? JSON.parse(v.image) : [];
                    res.rows[i].master = 0;
