    ...
    <AppLayout>
      <template #modal>
        ...
        <div class="modal" v-if="base.modal">
          <div
            class="modal-box relative w-11/12 md:w-6/12 lg:w-5/12 xl:w-4/12 max-w-5xl p-2 lg:p-4 max-h-screen"
          >
            <label
              for="modal-base"
            ...
            >
              <div
                class="flex flex-col items-center gap-4 w-full border-opacity-50 lg:flex-row justify-around content-center lg:items-start"
              >
                <div class="form-control">
                  <div class="form-control">
                    <label class="label cursor-pointer">
                      <span class="label-text">Visitor</span>
                      <input
                        type="checkbox"
                        class="toggle"
                        checked="checked"
                  ...
                  <div v-if="visitor">
                    <div class="form-control">
                      ...
                      <input
                        ...
                        v-model="base.form.user_ref"
                      />
                      <datalist id="visitors">
                        <option
                          :value="v.user_ref"
                          v-for="(v, i) in user_list.rows.filter(
                            (v) => v.user == 'visitor'
                          )"
                          :key="v"
                        ></option>
                  ...
                  <div v-else>
                    <div class="form-control">
                      <AppModuleGlobalSelectSearch
                        ...
                        @updateValue="
                          (obj) => {
                            base.form.user_data = obj;
                            base.form.user = obj.code;
                          }
                        "
          ...
          >
            ...
            <div class="card-body overflow-auto p-0 lg:p-0 max-h-[60vh]">
              <select
                ...
                v-model="visitor_select"
              >
                <option disabled selected value="null">Select visitor</option>
                <option
                  v-for="(v, i) in user_list.rows"
                  :key="v.code"
                  :value="v"
                >
                  {{ v.user_fullname || v.user_ref }}
                </option>
              </select>

              <div class="form-control">
                <label class="label">
                  <span class="label-text">Sum Ticket</span>
                </label>
              ...
              <div class="form-control p-2 pt-0">
                <label class="label">
                  <span class="label-text">Remark</span>
                </label>
                <textarea
                  class="textarea textarea-bordered border-base-content w-full"
                  placeholder="Remark"
                  v-model="detail.form.remark"
                ></textarea>
              </div>
            </div>
            <div class="backdrop-blur sticky top-0 items-center gap-2 flex">
              ...
              <div class="flex-1 form-control mt-1 mb-0">
                <button
                  class="btn btn-primary text-white"
                  @click="detail_save('static')"
                  :disabled="
                    !detail.form.ticket_no_first ||
                    !detail.form.ticket_no_last ||
                    sum_ticket == 0
                  "
                >
                  Confirm
          ...
          <div class="card shadow-lg bg-base-100">
            <div class="card-body overflow-auto p-2 lg:p-4">
              ...
              >
                <label
                  for="modal-detail"
                  class="btn btn-outline btn-primary btn-sm md:join-item text-white w-full md:w-fit"
                  @click="detail_create"
                >
                  GET TICKET
                </label>
                              ...
                              >
                                ...
                                <tbody>
                                  ...
                                  >
                                    ...
                                    <!-- <th>{{ v.code }}</th>
                                    <td>{{ v.user_fullname?v.user_fullname:v.user_ref }}</td> -->
                                    <td>{{ v.ticket }}</td>
                                    <td>
                                      {{ v.ticket_no_first }} -
                                      {{ v.ticket_no_last }}
                                    ...
                                    <td > 
                                      <div class="  max-w-40 overflow-auto max-h-10 text-[10px] border bg-base-100" v-if="v.remark ">
                                      <span class=" whitespace-pre-line ">{{ v.remark }}</span>
                                    </div>
                                  </td>
                                  </tr>
                                </tbody>
...
export default {
  ...
  methods: {
    changeTab(t) {
      this.base.page = 1;
      this.detail.page = 1;
      this[`${t == "Timestamps" ? "base" : "detail"}_search`]();
    },
    change() {
      this.detail.page = 1;
      this.detail_search();
    ...
    user_list_get(callback) {
      new Query("lastticket", "get").get(
        this,
        `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/TRR/timestamp?total=1&lastticket=1&groupuser=1&today=1`,
        (res) => {
          return callback({ ...res });
        }
      );
    ...
    base_get(callback) {
      new Query("base", "get").get(
        ...
        `${
          this.serviceUrl
        }api/controllers/MYSQL/INTERNAL/TRR/timestamp?total=1&today=1&page=${
          this.base.page
    ...
    base_create() {
      ...
      this.backward = false;
      this.user_list_get((res) => {
        this.user_list.rows = res.rows;
        this.user_list.total = res.total;
        this.user_list.next =
          this.user_list.page * this.user_list.row >= this.user_list.total
            ? false
            : true;
        this.user_list.back = this.user_list.page > 1 ? true : false;
        this.user_list.loading = false;
      });
    },
    base_save() {
      if (this.visitor) {
        this.base.form.user = "visitor";
      } else {
        this.base.form.user_ref = this.user.code;
      }
      let obj = {
      ...
      new Query("base", this.base.controll == "create" ? "POST" : "PUT").set(
        this,
        `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/TRR/timestamp?admin=1`,
        obj,
        (res) => {
          if (!res.success) {
      ...
      this.lastticket_get(
        (res) =>
          (this.lastticket.data =
            res.rows.length > 0 ? res.rows[0].ticket_no_last : "")
      );
      this.user_list_get((res) => {
        this.user_list.rows = res.rows;
        this.user_list.total = res.total;
        ...
        rows: [
          {
            ...
            ticket: this.sum_ticket,
            user: this.visitor_select.user,
            user_ref: this.visitor_select.user_ref,
          },
        ],
      };
    ...
    dashboard_get(callback) {
      new Query("dashboard", "get").get(
        ...
        `${
          this.serviceUrl
        }api/controllers/MYSQL/INTERNAL/TRR/ticket?dashboard=1&dash_from=${
          this.date.from
  ...
  watch: {
    ...
    visitor_select: function (v) {
      if (v) {
        ...
        // } else {
        //   this.detail.form.ticket_no_last = num + (this.sum_ticket-1);
        // }
        this.detail.form.ticket_no_first = parseInt(this.lastticket.data) + 1
        this.detail.form.ticket_no_last = parseInt(this.detail.form.ticket_no_first) + this.sum_ticket-1
      } else {
        this.sum_ticket = 0;
      }
    },
