                    ...
                    >
                      <thead>
                        <tr>
                          ...
                          <th class="text-right" v-if="base.form.doc_status == 'approve'">ลบรายการ</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr
                          ...
                          >
                            ...
                            <div v-else>
                              <label
                                ...
                                "
                                v-if="user.access.WarehouseIndirect.WHIndirectIssue == 'superadmin'"
                              >
                                ลบ
                              </label>
                ...
                <table class="table table-xs table-pin-rows table-pin-cols table-zebra" v-else>
                  ...
                  <tbody>
                    <tr v-for="(v, i) in base.rows" :key="v.code" class="hover" >
                      ...
                      <th class="text-center" v-if="v.doc_status != 'close'">
                        <label
                          for="modal-detail"
...
export default {
  ...
  methods: {
    exportExcel() {
      return window.open(`${
        this.serviceUrl
      }api/controllers/MYSQL/INTERNAL/WHI/exports?for=issue_item&total=1&approve=1&noreject=1${this.user.access.WarehouseIndirect.WHIndirectRequest == 'user' ? `&su=${this.user.code}` 
    ...
    base_get(callback) {
      new Query('base','get').get(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WHI/request_item?total=1&approve=1&noreject=1${
    ...
    detail_save(type) {
      ...
      new Query(null, this.detail.controll == "create" ? "POST" : "PUT").set(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WHI/transaction`, obj, (res) => {
        if (!res.success) {
          ...
          if (parseInt(res.rows[0].amount) <= parseInt(res.rows[0].min)) {
            let data = {
              mailSubject : '[Alert] Item ในคลัง Indirect มีจำนวนคงเหลือที่ น้อยกว่า/เท่ากับ จำนวนขั้นต่ำที่ตั้งเอาไว้แล้ว',
    ...
    user_dept(callback) {
      if (this.user.department == '170564984084c6f') {
        this.dept = 'im';
      }
      else if (this.user.department == '17056498404bf20') {
        this.dept = 'pdc';
      }
      else if (this.user.department == '170564984092be0') {
        this.dept = 'pdl';
      }
      else if (
        this.user.department == '17056498405bb45' || this.user.department == '1721103490tpyEU' || this.user.department == '1721103499WUnje'
        || this.user.department == '1726632913KvP0Q' || this.user.department == '1705649840cabf2' || this.user.department == '17056498404fe0b'
      ) {
        this.dept = 'qaqc';
      }
      else if (this.user.department == '1705649840b0cd6' || this.user.department == '1726632979kwGly' || this.user.department == '1726633003e4wZW') {
        this.dept = 'rd';
      }
      else { this.dept = 'it' }
