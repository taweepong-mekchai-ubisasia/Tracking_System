...
{{ label }} -->
        <ul
          class="absolute menu menu-xs bg-base-100 border-2 w-full shadow-lg max-h-60 overflow-auto block"
        ...
        >
          <li v-for="(v, i) in base.rows" :key="i" @click="setValue(v)">
            <a :class="`${v[code] == current?'active':''}`">{{ v[label] }}</a>
          </li>
        </ul>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    this.base.current[this.label] = this.current
    ...
    refresh: function (val) {
      if (val) {
        ...
        this.base_search((rows) => {
          ...
          this.base.current  = this.base.rows.find((v, i) => v[this.code]==this.current);
        });
      } else {
      }
    },
