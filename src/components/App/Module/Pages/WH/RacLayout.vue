<template>
  <div class="RacLayoutModule">
    ...
    <div class="flex relative flex-row flex-wrap justify-center content-center">
      <!-- <div class="w-full h-[80vh] content-center" v-if="base.loading">
        <span class="loading loading-spinner loading-lg text-warning"></span>
      </div> -->
      <!-- <div class="w-full h-[80vh] relative overflow-auto" v-else> -->
...
export default {
  ...
  methods: {
    ...
    base_get(callback) {
      new Query('base','get').get(this, `${
          this.serviceUrl
        }api/controllers/MYSQL/INTERNAL/WH/layout?debug=1&wh=${this.wh}&transref=I&page=${
          this.base.page
    ...
    base_save() {
      ...
      new Query('base', this.base.controll == "create" ? "POST" : "PUT").set(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WH/layout`, ...Object.assign({}, this.base.rows, { wh: this.wh }), (res) => {
        if (!res.success) {
        // localStorage.removeItem("user_token");
        // this.$router.push({ name: `Login` });
        } else {
    ...
    rac_search(callback) {
      ...
      this.rac_get((res) => {
        this.rac.rows = res.shlf;
        this.base.rows = res.lay;
    ...
    rac_get(callback) {
      new Query('rac','get').get(this, `${
          this.serviceUrl
        }api/controllers/MYSQL/INTERNAL/WH/shelf_layout?action=count&transref=I&transref_type_null=1&wh=${
          this.wh
  ...
  mounted() {
    this.$nextTick(() =>{
      ...
      // this.rac_search((res) => this.configData())
    });
  },
  created() {},
  beforeDestroy() {},
