<template>
  <div class="card card-compact shadow-lg bg-base-100 border-2">
    <div class="card-body overflow-auto">
      ...
      <div class="overflow-x-auto w-full max-h-[64vh]">
        ...
        <table class="table table-xs table-pin-rows table-pin-cols table-zebra" v-else>
          <thead>
            <tr class="italic">
              ...
              <td>Item Ref</td>
              <td>Doc Ref</td>
              <td>ผู้ทำรายการ (request)</td>
              <td class="text-right">Quantity</td>
              <td class="text-right">Price</td>
              <td class="text-right">Total Cost</td>
              <td>Comment</td>
              <td>วันเวลา import/issue</td>
              <td>ผู้ทำรายการ (import/issue)</td>
              <td>Deletion</td>
            </tr>
          </thead>
          <tbody>
            ...
            >
              ...
              <td>
                <div class="flex items-center space-x-3">
                  <div>
                    ...
                    <div class="text-xs opacity-70" v-if="v.requester_fullname">
                      ( {{
                        v.req_dept
                          ? v.req_dept
                          : "-"
              ...
              <td class="text-right">{{ new Intl.NumberFormat().format(v.qty) }}</td>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      return window.open(`${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WHI/exports?for=transaction&total=1${
        this.user.access.WarehouseIndirect.WHIndirectReport != 'superadmin' ? `&dept=${this.user.department}` : ''}${
      this.date.from ? `&from=${this.date.from}` : ''}${this.date.to ? `&to=${this.date.to}` : ''}${this.base.q ? `&q=${this.base.q}` : ""}`);
    },
    change() {
      this.base_search();
    },
    ...
    base_get(callback) {
      ...
      new Query('base','get').get(this, `${this.serviceUrl}api/controllers/MYSQL/INTERNAL/WHI/transaction?total=1${
        this.user.access.WarehouseIndirect.WHIndirectReport != 'superadmin' ? `&dept=${this.user.department}` : ''}
      &action=all${this.date.from ? `&from=${this.date.from}` : ''}${this.date.to ? `&to=${this.date.to}` : ''}&page=${this.base.page}${
