<template>
  <div>
    <button
      ...
      :class="{ '': isOpen, 'bg-blue-500 border-blue-500': !isOpen }"
      @click="toggleDrawer"
    >
      <Icon icon="tabler:search" class="w-4 h-4 block lg:hidden" />
      <span class="hidden lg:block w-12">Search</span>
      <span
        v-if="isOpen"
        class="absolute top-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full"
      ></span>
    </button>
    <teleport to="#app">
      <div v-if="isOpen">
        ...
        >
          ...
          <div
            class="relative p-4 overflow-y-auto h-[calc(100%-60px)] overflow-auto"
          >
            <div class="grid gap-y-4 gap-x-2 w-full h-full grid-cols-1">
              <div v-for="(v, i) in base" :key="i" class="relative">
                ...
                <template v-else>
                  <label
                    class="absolute top-[-0.5rem] left-[0.65rem] text-xs bg-white z-10 font-bold text-opacity-50"
                  ...
                  <label
                    class="absolute left-[0.65rem] text-xs bg-white z-10 font-bold text-opacity-50"
                    v-else
                  >
                    {{ v.label }}
                  </label>
                  <select
                    class="select select-xs sm:select-sm select-bordered border-gray-300 w-full"
                    :class="class"
                    @input="handleInput"
                    v-model="v.value"
                    v-if="v.type == 'select'"
                  >
                    <option class="text-xs" selected value="">All</option>
                    <option class="text-xs" v-for="val in v.child" :value="val.value">{{ val.label }}</option>
                  </select>
                  <input
                    :type="v.type"
                    :placeholder="v.placeholder"
                    class="checkbox checkbox-xs sm:checkbox-sm float-end border-base-content"
                    :class="class"
                    style="width: 30px;"
                    :checked="v.check"
                    @input="handleInput"
                  ...
                  <input
                    ...
                    v-else
                  />
                  <!-- {{ user.access.IM.IMReportExpiryInspec == 'superadmin'}} -->
                </template>
              </div>
              <div>
                <button
...
const touchStartY = ref(0); // บันทึกตำแหน่งสัมผัสเริ่มต้น
const deltaY = ref(null); // ใช้สำหรับเก็บทิศทางการเคลื่อนไหว
const lastHeight = ref(null); // ใช้สำหรับเก็บทิศทางการเคลื่อนไหว
const screenHeight = ref(window.innerHeight); // ใช้สำหรับเก็บทิศทางการเคลื่อนไหว
...
base.value.forEach((v) => {
  if (v.type == "combobox") {
    // v.component = defineAsyncComponent(() =>
    //   import(`@/components/App/Module/Global/Combobox.vue`)
    // );
    v.component = markRaw(
      defineAsyncComponent(() =>
        import(`@/components/App/Module/Global/Combobox.vue`)
