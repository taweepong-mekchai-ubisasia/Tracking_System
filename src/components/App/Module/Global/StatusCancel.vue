      ...
      <div class="modal-box relative p-0">
        ...
        >
          <div class="avatar placeholder place-content-center mb-4">
            <div class="w-24 rounded-full border-4 border-gray-500">
          ...
          <h3 class="text-lg font-bold text-gray-500">
            Are your sure for <span class="underline underline-offset-2 decoration-double">Cancel</span> this item?
          </h3>
        </div>
        ...
        <div class="backdrop-blur sticky top-0 items-center gap-2 p-4 flex">
          ...
          <div class="flex-1 form-control">
            <button
              class="btn btn-xs sm:btn-sm md:btn-md bg-gray-500 text-white"
              @click="confirm()"
              :disabled="base.loading"
            >
              Confirm
      ...
      try {
        const body = {
          ...
          rows: base.value.rows
            ? base.value.rows
            : [{ code: base.value.code, process: "cancel" }],
        };
