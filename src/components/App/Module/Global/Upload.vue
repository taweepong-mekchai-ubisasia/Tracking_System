      ...
      >
        ...
        >
          <img
            width="auto"
            height="auto"
            class="max-h-full object-cover bg-cover"
            :class="fileLink.indexOf('employee') > -1 ? 'max-h-full' : 'max-h-44'"
            v-if="file.length > 0"
    ...
    <div class="overflow-auto min-h-[15vh] max-h-[15vh]" v-if="multiple && type == 'pdf'">
      <table class="table table-xs table-pin-rows table-pin-cols">
        <thead>
          <tr>
            <th>#</th>
            <td>{{ type == 'image' ? 'Name' : 'Document Name' }}</td>
            <!-- <td>Upload</td> -->
        ...
        <tbody v-for="(v, i) in file" :key="i">
          ...
          <tr>
            <th>{{ i+1 }}</th>
            <td class="crop w-full max-w-96">
              <a class="btn btn-link btn-xs" :href="!v.temp?`${fileLink}/${v.file}` : `${serviceUrl}tmps/pdf/${v.file}`" target="_blank">{{ v.name }}</a>
              <!-- <span v-else>{{ v.file }}</span> -->
            </td>
            ...
            <th class="text-center">
              <progress
                ...
                max="100"
              ></progress>
              <div class="flex justify-center text-success" v-else>
